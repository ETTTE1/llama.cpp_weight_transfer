...
static int llama_model_load(const std::string & fname, std::vector<std::string> & splits, llama_model & model, llama_model_params & params) {
    ...
    try {
        ...
        // ==================== [DEBUG CONFIG GENERATION START] ====================
        std::vector<int32_t> debug_config_data; 
        struct ggml_context * debug_ctx = nullptr;

        auto setup_debug_ffn_config = [&](int n_layers) -> ggml_tensor * {
            // 1. simulation: even layers for CPU (1)，odd layers for GPU (0)
            debug_config_data.resize(n_layers);
            for (int i = 0; i < n_layers; ++i) {
                if (i % 2 == 0) {
                    debug_config_data[i] = 1; // Even: CPU
                } else {
                    debug_config_data[i] = 0; // Odd: GPU
                }
            }

            // 2. create a GGML context for the Tensor
            struct ggml_init_params params = {
                /*.mem_size   =*/ ggml_tensor_overhead() + 256,
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true, 
            };
            debug_ctx = ggml_init(params);
            if (!debug_ctx) {
                LLAMA_LOG_INFO("%s: [DEBUG] failed to init context for config\n", __func__);
                return nullptr;
            }

            // 3. create the Tensor [n_layers]
            ggml_tensor * t = ggml_new_tensor_1d(debug_ctx, GGML_TYPE_I32, n_layers);
            
            t->data = debug_config_data.data();
            
            LLAMA_LOG_INFO("%s: [DEBUG] Generated FFN config for %d layers (Even=CPU, Odd=GPU)\n", __func__, n_layers);
            
            return t;
        };

        ml.ffn_layer_config = setup_debug_ffn_config(model.hparams.n_layer);
        // ==================== [DEBUG CONFIG GENERATION END] ====================
