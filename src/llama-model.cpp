...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            // logic: if the current tensor is FFN and config requires CPU, override the buft_list
            
            bool logic_overridden = false; // for debug/logging purposes

            // only for Repeating layer tensors and if ffn_layer_config is provided
            if (ml.ffn_layer_config && ml.ffn_layer_config->data && info.layer == LLM_TENSOR_LAYER_REPEATING) {
                
                std::string tensor_name = tn.str();

                bool is_ffn = (tensor_name.find("ffn") != std::string::npos);

                if (is_ffn) {
                    const int32_t * config_map = (const int32_t *) ml.ffn_layer_config->data;
                    int64_t n_config_layers = ml.ffn_layer_config->ne[0];

                    // modify buft_list if needed
                    if (tn.bid >= 0 && tn.bid < n_config_layers) {
                        // 1 for CPU
                        if (config_map[tn.bid] == 1) { 
                            buft_list = &pimpl->cpu_buft_list;
                            logic_overridden = true;
                        }
                    }
                }
            }
            ...
            // ==================== [new logic START: print DEBUG log] ====================
            ggml_tensor * created_t = ml.create_tensor(ctx, tn, ne, flags);

            // if created and logic overridden, print logging info
            if (created_t && logic_overridden) {
                const char * actual_buf_name = "Unknown";
                if (created_t->buffer) {
                    actual_buf_name = ggml_backend_buffer_name(created_t->buffer);
                } else if (buft) {
                    actual_buf_name = ggml_backend_buft_name(buft);
                }

                LLAMA_LOG_INFO("[Hybrid-Split] Layer %d Tensor %s -> Placed on %s (Forced CPU)\n", 
                    tn.bid, tn.str().c_str(), actual_buf_name);
            }
